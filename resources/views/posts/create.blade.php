<x-app-layout>
	...
	<h1 class="p-4 text-center text-2xl font-extrabold text-zinc-700">Add New Post</h1>

	<div class="p-4 flex flex-col flex-wrap gap-8 justify-center">


		<form action="{{ route('posts.store') }}" class="max-w-sm mx-auto" novalidate method="post"
			...
			<div class="mb-5">
				...
				<input type="text" id="title" name="title"
					...
					placeholder="Your Post Title" required value="{{ old('title') }}" />
			...
			<div class="mb-5">
				...
				<textarea id="body" name="body"
					class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
					required placeholder="Your Post Content">{{ old('body') }}</textarea>
				<small class="text-xs text-red-700">
					@error('body')
						{{ $message }}
					@enderror
			...
			<div class="mb-5">
				...
				<select id="post_status_id" name="post_status_id"
					...
					@foreach ($post_statuses as $post_status)
						<option {{ old('post_status_id') == $post_status->id ? 'selected' : ''}}
			...
			<!-- 
			<select>
				<option {{ old('post_status_id') == 1 ? 'selected' : ''}} value="">A</option>
				<option {{ old('post_status_id') == 2 ? 'selected' : ''}} value="">B</option>
				<option {{ old('post_status_id') == 3 ? 'selected' : '' }} value="">C</option>
				<option {{ old('post_status_id') == 4 ? 'selected' : ''}} value="">D</option>
			</select> -->
